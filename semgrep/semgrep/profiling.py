...
class ProfilingData:
    ...
    def init_empty(self, rules: List[core.RuleId], targets: List[Path]) -> None:
        ...
        self._file_parse_time = defaultdict(float)
        self._file_run_time = defaultdict(float)
        self._match_time_matrix = defaultdict(Times)

        self._rule_match_times = defaultdict(float)
        self._rule_bytes_scanned = defaultdict(int)

        self._file_match_times = defaultdict(float)
        self._file_num_times_scanned = defaultdict(int)
